...
export async function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-line/60 bg-bg/70 backdrop-blur-xl">
      ...
      {/* Navigazione compatta su mobile (le voci sopra sono nascoste) */}
      <nav className="flex items-center justify-center gap-7 border-t border-line/50 py-2.5 md:hidden">
        {NAV.map((l) => (
          <Link
            key={l.href}
            href={l.href}
            className="signage text-[12px] text-muted transition-colors hover:text-ink"
          >
            {l.label}
          </Link>
        ))}
